...
echo "information needed for base score production ... initializing"

rm serverInfo2.txt
nmap -A -sV -T5 -p- --version-all "$1" >> serverInfo2.txt

rm Domain.txt
#cat serverInfo.txt | grep 'open  domain' | cut -d ' ' -f 8-10  --output-delimiter=+ >> Domain.txt
cat serverInfo2.txt | grep 'open  domain ' | awk -F'open  domain  ' '{print $2}' | cut -f1 -d"(" >> Domain.txt
sed -i 's/ /+/g' Domain.txt && sed 's/.$//' Domain.txt

rm Smtp.txt
cat serverInfo2.txt | grep 'open  smtp' | cut -d ' ' -f 8 >> Smtp.txt
...
./portScan -ip "$1" >> strangePortOut.txt
